...
- 담당자 : 김세현
    ...
    - Gateway에서 API 요청을 분석하여 인증 필요 여부(member/guest)를 판단하고 적절한 서비스로 분기 처리
    - JWT 토큰 기반 회원 인증/인가 시스템 구축
    - OAuth2 Google, Payco 로그인 구현
    - 포인트 적립 및 사용, 환불 로직 구현
...
### **🔍 검색**
- 담당자 : 김근오
- ElasticSearch 기반 도서 검색(BM25) 기능 구현 및 성능 최적화
- ElasticSearch 기반 도서 AI 검색(Embedding Vector + Rerank + LLM ) 구현
- Reranker + Gemini LLM으로 상위 결과 재정렬/검증 및 추천 이유 생성
- 장애 대비 BM25 fallback, TopK 제한/타임아웃/캐시 적용으로 검색 속도 및 안정성 확보
- 도서 데이터 ElasticSearch 에 인덱싱/동기화(Upsert/Delete) 기능 구현
...
<tr>
    ...
    <td colspan="2" rowspan="3">이번 프로젝트에서 가장 까다로웠던 점은 검색 품질과 성능·비용을 동시에 만족시키는 균형을 잡는 일이었습니다. BM25 키워드 검색만으로는 표현이 다른 질의에서 누락이 생겼고, 이를 보완하기 위해 임베딩 기반 벡터 검색과 Reranker, LLM을 단계적으로 결합하며 정확도를 끌어올렸습니다. 하지만 AI 검색은 지연과 비용이 발생하기 때문에 TopK 제한, 캐시, 타임아웃, 장애 시 BM25로의 fallback 같은 운영 관점의 안전장치도 함께 설계해야 했습니다. 또한 도서 데이터의 변경을 빠르게 반영하기 위해 단건 인덱싱과 동기화 로직을 구현하면서, 검색 기능은 결국 데이터 최신성과 예외 처리까지 포함한 전체 흐름 위에서 완성된다는 점을 체감했습니다. 결과적으로 단순히 검색이 되는 수준이 아니라, 실제 서비스에서 신뢰할 수 있는 검색 경험을 만들기 위해 디테일을 챙기는 방법을 배운 값진 경험이었습니다.</td>
  </tr>
  <tr></tr>
  <tr></tr>
  ...
  <tr>
    ...
    <td colspan="2" rowspan="3">이번 프로젝트에서 회원, 인증, 포인트 도메인을 책임지며, 게이트웨이 부하 분산부터 데이터 정합성 확보까지 백엔드의 핵심적인 설계 과제들을 깊이 있게 경험했습니다. 특히 초기 단계에서 게이트웨이에 집중된 인증 로직을 별도 서버로 분리하는 책임 분리를 단행하고, JWT의 보안 취약점을 보완하는 과정에서 한 단계 성장할 수 있었습니다. 또한, 분산 환경에서 포인트 데이터의 무결성을 지키기 위해 보상 트랜잭션 로직을 설계하며, 예외 상황에서도 데이터가 안전하게 관리될 수 있는 구조를 찾기 위해 노력했습니다. 단일 기능 구현에 매몰되지 않고 시스템 전체를 이해하려는 시야를 가질 수 있었던 값진 시간이었으며, 무엇보다 팀원들과 지식을 나누며 함께 정답을 찾아가는 협업의 즐거움을 배운 소중한 경험이었습니다.</td>
  </tr>
  <tr></tr>
  <tr></tr>
